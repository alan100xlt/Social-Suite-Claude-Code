...
const platformIcons: Record<Platform, React.ElementType> = {
  instagram: FaInstagram, twitter: FaTwitter, facebook: FaFacebook, linkedin: FaLinkedin,
  tiktok: FaTiktok, youtube: FaYoutube, pinterest: FaYoutube, reddit: FaYoutube,
  bluesky: SiBluesky, threads: SiThreads, "google-business": FaFacebook, telegram: FaFacebook, snapchat: FaFacebook,
};

const platformColors: Record<Platform, string> = {
  instagram: "bg-instagram", twitter: "bg-twitter", facebook: "bg-facebook", linkedin: "bg-linkedin",
  tiktok: "bg-tiktok", youtube: "bg-destructive", pinterest: "bg-destructive", reddit: "bg-destructive",
  bluesky: "bg-twitter", threads: "bg-foreground", "google-business": "bg-success", telegram: "bg-twitter", snapchat: "bg-warning",
};
...
export function CalendarTab() {
  ...
  const getPostPlatform = (post: GetLatePost): Platform => {
    if (post.platformResults && post.platformResults.length > 0) return post.platformResults[0].platform;
  ...
  const sortedPosts = [...(posts || [])].sort((a, b) => new Date(a.scheduledFor || 0).getTime() - new Date(b.scheduledFor || 0).getTime());
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="font-normal">{weekLabel}</Badge>
            </div>
          </CardHeader>
          <CardContent>
              ...
              <div className="min-w-[600px]">
                <div className="grid grid-cols-7 gap-2 mb-4">
                  {weekDates.map((date, i) => {
                    ...
                    return (
                      <div key={i} className={cn("text-center py-2 rounded-lg", isToday && "bg-accent/10")}>
                        <p className="text-xs text-muted-foreground uppercase tracking-wide">{daysOfWeek[i]}</p>
                  ...
                  {hours.slice(0, 6).map((hour) => (
                    <div key={hour} className="grid grid-cols-7 gap-2">
                      {weekDates.map((date, i) => {
                        ...
                        return (
                          <div key={`${i}-${hour}`} className={cn("h-12 rounded-lg border border-border/50 flex items-center justify-center text-xs", post && `${colorClass} text-white border-transparent`)}>
                            {post && Icon && (
                              <div className="flex items-center gap-1 px-2">
                                <Icon className="w-3 h-3" />
                                <span className="truncate max-w-[60px]">{format(parseISO(post.scheduledFor!), "h:mm a")}</span>
                              </div>
                            )}
                          </div>
                        );
          ...
          <CardContent className="space-y-4">
            {sortedPosts.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-primary mt-2">Use the Compose tab to schedule your first post</p>
              </div>
            ) : (
              sortedPosts.map((post) => {
                ...
                return (
                  <div key={post.id} className="flex gap-3 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors cursor-pointer animate-fade-in">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {post.scheduledFor ? format(parseISO(post.scheduledFor), "MMM d 'at' h:mm a") : "Not scheduled"}
                      </p>
                    </div>
                  </div>
                );
